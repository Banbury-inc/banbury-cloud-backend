...
def search_s3_files(username, query):
    """
    Searches S3 files for a specific user by query string.
    
    Args:
        username (str): The username whose S3 files to search
        query (str): The search query to match against file names
        
    Returns:
        dict: A dictionary containing the list of matching files and any error message
    """
    try:
        # Connect to MongoDB
        ...
        # This will match files that contain the query string anywhere in the name
        regex_pattern = re.compile(re.escape(query), re.IGNORECASE)
        
        # Find all files with s3_url field that belong to the user and match the query
        ...
        print(f"Error searching S3 files: {e}")
        return {"error": f"Failed to search S3 files: {str(e)}", "status_code": 500}
